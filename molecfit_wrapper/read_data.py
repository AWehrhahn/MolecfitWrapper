...
def read_ESPRESSO_S2D(
    inpath, outname, air=True, nowave=False, molecfit=True, mode="HARPS"
):
    ...
    if os.path.exists(inpath) != True:
        print(
            "ERROR in read_ESPRESSO_S2D: Data input path (%s) does not exist." % inpath
        )
        sys.exit()
    ...
    if ccf_count != sci_count:
        print(
            "ERROR in read_ESPRESSO_S2D: There is a different number of science CCFs as there is science frames."
        )
        sys.exit()
    ...
    if e2ds_count == 0:
        print(
            "ERROR in read_ESPRESSO_S2D: The input folder (%s) does not contain files ending in e2ds.fits."
            % inpath
        )
        sys.exit()
    if sci_count == 0:
        print(
            "ERROR in read_ESPRESSO_S2D: The input folder (%2) contains e2ds files, but none of them are classified as SCIENCE frames with the HIERARCH ESO DPR CATG/OBS-TYPE keyword."
        )
        print("These are the files and their types:")
        for i in range(len(type)):
            print("   " + framename[i] + "  %s" % type[i])
    ...
    if np.max(np.abs(norders - norders[0])) == 0:
        ...
        print(
            "ERROR in read_ESPRESSO_S2D: Not all files have the same number of orders."
        )
    ...
    if np.max(np.abs(npx - npx[0])) == 0:
        ...
        print(
            "ERROR IN read_ESPRESSO_S2D: Not all files have the same number of pixels."
        )
    ...
    if np.shape(wave) != np.shape(e2ds[0]):
        print(
            "ERROR in read_ESPRESSO_S2D: A wave file was detected but its shape (%s,%s) does not match that of the orders (%s,%s)"
            % (
                np.shape(wave)[0],
                np.shape(wave)[1],
                np.shape(e2ds[0])[0],
    ...
    if len(s1dhdr) != len(e2ds) and molecfit == True:
        print(
            "ERROR in read_HARPS_e2ds: The number of s1d SCIENCE files and e2ds SCIENCE files is not the same. (%s vs %s)"
            % (len(s1dhdr), len(e2ds))
        )
    ...
    if molecfit == True:
        ...
        #
        # sys.exit()
        list_of_wls, list_of_trans = mol.do_molecfit(
            s1dhdr_sorted, s1d_sorted, wave=wave1d, load_previous=False, mode=mode
        )
        mol.write_telluric_transmission_to_file(
            list_of_wls, list_of_trans, outpath + "telluric_transmission_spectra.pkl"
        )
...
def read_ESPRESSO_S2D_JULIA(inpath, outname, air=True, molecfit=True):
        ...
        if filelist[i].endswith("S2D_A.fits"):
            ...
            if hdr2["EXTNAME"] == "SCIDATA":
                ...
                airmass = np.append(
                    ...
                    0.5
                    * (
                        hdr1["HIERARCH ESO TEL3 AIRM START"]
                        + hdr1["HIERARCH ESO TEL3 AIRM END"]
                    ),
                )
    ...
    for i in range(int(norders[0])):
        ...
        for j in range(len(s2dsorting)):  # Loop over exposures
            ...
            if i == 0:  # Only do it the first time, not for every order.
                line = (
                    str(mjd[s2dsorting[j]])
                    + "\t"
                    + date[s2dsorting[j]]
                    + "\t"
                    + str(texp[s2dsorting[j]])
                    + "\t"
                    + str(airmass[s2dsorting[j]])
                    + "\t"
                    + str(berv[s2dsorting[j]])
                    + "\t"
                    + framename[s2dsorting[j]]
                    + "\n"
                )
                f.write(line)
            c += 1
