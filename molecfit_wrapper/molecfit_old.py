...
# TODO: these appear to be custom modules?
# import utils as ut
...
def do_molecfit(headers, spectra, wave=[], mode="HARPS", load_previous=False):
    ...
    molecfit_input_folder = (
        "/Users/hoeijmakers/Molecfit/share/molecfit/spectra/cross_cor/"
    )
    molecfit_prog_folder = "/home/ansgar/ESO/molecfit/bin/"
    temp_specname = copy.deepcopy(
        mode
    )  # The name of the temporary file used (without extension).
    ...
    if load_previous == False:
        ...
        for i in range(N):  # range(len(spectra)):
            ...
            execute_molecfit(
                molecfit_prog_root, molecfit_input_root + parname, gui=False
            )
            wl, fx, trans = retrieve_output_molecfit(
                molecfit_input_root + temp_specname
            )
            remove_output_molecfit(molecfit_input_root, temp_specname)
    ...
    if len(to_do_manually) > 0:
        ...
        for i in to_do_manually:
            ...
            execute_molecfit(
                molecfit_prog_root, molecfit_input_root + parname, gui=True
            )
            wl, fx, trans = retrieve_output_molecfit(
                molecfit_input_root + temp_specname
            )
...
class molecfit_gui(object):
    ...
    def __init__(self, wls, fxc, trans):
        ...
        plt.subplots_adjust(
            left=0.05
        )  # Create space for the interface (#rhyme comments ftw <3)
        plt.subplots_adjust(right=0.75)
        plt.subplots_adjust(bottom=0.1 + 0.05 * self.nrows)
        plt.subplots_adjust(top=0.95)
        self.set_spectrum(self.i)
        self.ax[0].set_title(
            "Corrected / uncorrected s1d #%s/%s" % (self.i, self.N - 1)
        )  # start counting at 0.
        self.ax[1].set_title("Transmission spectrum")
        self.img1a = self.ax[0].plot(
            wls[self.i], fxc[self.i] * trans[self.i], alpha=0.5
        )
        ...
        self.crosses = (
            []
        )  # List of checkboxes that have crosses in them. Starts empty, too.
    ...
    def set_spectrum(self, i):
        ...
        self.img_max = (
            np.nanmean(self.spectrum[fun.selmax(self.spectrum, 0.02, s=0.02)]) * 1.3
        )
    ...
    def previous(self, event):
        ...
        if (
            self.i < 0
        ):  # If order tries to become less than zero, loop to the highest spectrum.
            self.i = self.N - 1
        self.set_spectrum(self.i)
...
def check_fit_gui(wls, fxc, trans):
    ...
    M.spectrum_slider = Slider(
        rax_slider, "", 0, M.N - 1, valinit=0, valstep=1
    )  # Store the slider in the model class
    ...
    for i in range(M.N):
        ...
        if float(i) - offset > M.maxboxes - 1.0:
            ...
            offset += M.maxboxes
        M.selec.plot(
            float(i) - offset + np.array([-0.5, -0.5, 0.5, 0.5, -0.5]),
    ...
    M.click_connector = M.fig.canvas.mpl_connect(
        "button_press_event", select_spectrum_box
    )  # This is the connector that registers clicks
...
def execute_molecfit(molecfit_prog_root, molecfit_input_file, gui=False):
    ...
    if gui == True:
        command = (
            "python3 " + molecfit_prog_root + "molecfit_gui " + molecfit_input_file
        )
        os.system(command)
    # python3 /Users/hoeijmakers/Molecfit/bin/molecfit_gui /Users/hoeijmakers/Molecfit/share/molecfit/spectra/cross_cor/test.par


def write_file_to_molecfit(
    molecfit_file_root, name, headers, spectra, ii, mode="HARPS", wave=[]
):
    ...
    if mode == "HARPS":
        ...
        wave = (
            headers[ii]["CDELT1"] * fun.findgen(len(spectra[ii]))
            + headers[ii]["CRVAL1"]
        ) * (1.0 - berv / const.c)
    elif mode == "ESPRESSO":
        if len(wave) == 0:
            print(
                "ERROR in WRITE_FILE_TO_MOLECFIT: WHEN MODE = ESPRESSO, THE 1D WAVE AXIS NEEDS TO BE PROVIDED."
            )
            sys.exit()
...
def apply_telluric_correction(inpath, list_of_wls, list_of_orders):
    ...
    if No != len(list_of_orders):
        print(
            "ERROR in telluric correction: List of data wls and List of orders do not have the same length."
        )
        sys.exit()
    ...
    if Nexp != len(fxT):
        print(
            "ERROR in telluric correction: List of telluric wls and telluric spectra read from file do not have the same length."
        )
        sys.exit()
